...
def aggregateLogs():
    totalInvalid = 0
    totalValid = 0
    with open("./data/log.txt", "r") as inputFile:
        for line in inputFile:
            splitLine = line.split()
            totalInvalid += int(splitLine[2])
            totalValid += int(splitLine[4])
    print(totalInvalid, totalValid, totalInvalid + totalValid)
